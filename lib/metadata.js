...
function buildOgImageUrl({ title, description, page, locale }) {
  const siteUrl = 'https://ghostseo.ai';
  const params = new URLSearchParams({
    title: title || 'GhostSEO',
...
export async function generatePageMetadata({ locale, page, useDraft = false, overrides = {} }) {
  ...
  const siteUrl = siteSeo.siteUrl || 'https://ghostseo.ai';
  const siteName = siteSeo.siteName?.[locale] || siteSeo.siteName?.en || 'GhostSEO';
...
export function getOrganizationJsonLd() {
  return {
    ...
    logo: 'https://ghostseo.ai/ghostseo_logo.png',
    description: 'AI-Powered SEO Automation Platform',
    sameAs: [
      'https://twitter.com/ghostseo',
      'https://linkedin.com/company/ghostseo'
    ],
    contactPoint: {
      '@type': 'ContactPoint',
      email: 'office@ghostseo.ai',
